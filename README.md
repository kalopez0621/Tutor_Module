...
The **AI-Powered Tutor Module** provides personalized, on-demand tutoring to college students by adapting to individual needs through AI-driven insights and recommendations. Inspired by Together AI's **LlamaTutor** framework and the adaptive-learning concepts of **OATutor**, this implementation runs entirely on a **local Gemma model via Ollama** — making it cost-free, private, and fully self-hosted.

### Why This Project Exists — Complementing Brainfuse Tutoring

Miami Dade College already offers **Brainfuse Tutoring**, a valuable academic resource with live help, writing labs, and subject-specific assistance. However, students often face barriers such as **limited availability, wait times, or difficulty articulating what to ask**. The Tutor Module is designed as a **front-line companion** to Brainfuse rather than a replacement:
...
- Enforces variety — explicit instructions to avoid reusing or rewording the original question
...
- Tutor Mode with follow-up flow ("Did you understand?" → Yes/Not Sure/No)
...
- **JSON-backed persistence:** Lightweight storage perfect for prototyping; ready to migrate to PostgreSQL/MongoDB for production.
...
Ollama exposes its API at `http://localhost:11434` — this is where `gemma_engine.py` sends requests.
...
### `main.py`
Entry point that initializes the FastAPI app, registers the router, and exposes a root health-check endpoint. On startup, it triggers a **model warm-up hook** — sending a dummy `"hello"` request to Gemma so the first real student request doesn't hit a cold-start delay.

### `routes.py`
The heart of the API. Handles:
- **Auto-detection** of subject (`detect_subject_from_question`) and concept (`extract_concept_from_question`) when the student does not specify them — useful for guest mode
- **`/tutor/query`** — orchestrates prompt building, Gemma call, history logging, and resource lookup
...
> **Model flexibility:** The current code targets `gemma:2b` for low-resource compatibility, but the module has been successfully tested with `gemma3:4b` and `gemma3:1b` as well. Swapping models is a one-line change in `gemma_engine.py` — useful when running on a more capable machine (e.g., during demos) where a larger model produces noticeably better tutoring quality.

### `prompt_engine.py`
The "secret sauce" of tutoring quality:

- **`build_prompt()`** — selects a subject-specific persona (math tutor, English writing coach, biology tutor, generalist) and embeds course, difficulty, and formatting rules (LaTeX for math, supportive feedback for English, real-world analogies for biology)
...
- **`get_recommendations()`** — course + topic → study suggestions
- **`get_resources()`** — subject + concept → curated Khan Academy/YouTube links
- **`get_visual_resources()`** — subject + concept → diagrams, interactive tools, videos
...
- **Smart rendering** — separates plain markdown from LaTeX blocks
- **Follow-up flow** — students can answer the tutor's check-in question and receive feedback
- **Three-button comprehension check** — "Yes, give me practice" / "Not sure" / "No, explain again"
- **Clarification loop** — sends a refined re-explanation request to the AI
- **Practice Mode** — hide/reveal answer with explanation
...
The Phase 1 codebase is intentionally modular so that the following enhancements can be added without major refactoring.
...
- 🔲 **Faculty Dashboard** — engagement metrics, common stumbling concepts, time-on-task
- 🔲 **Student Progress Reports** — strength/weakness breakdown, mastery percentages
- 🔲 **District-level Insights** — anonymized aggregates for administrators
...
- 🔲 Deploy to **AWS** / **Google Cloud** / **Azure** with auto-scaling
- 🔲 Replace local Ollama with a **GPU-backed inference server** for production load
- 🔲 Add **Redis** caching for frequent queries
- 🔲 Set up **CI/CD** pipeline with automated testing

### Quick-Win Improvements (Low-Hanging Fruit)
...
- **LlamaTutor** by Together AI — chatbot tutor architecture
- **OATutor** — adaptive learning recommendations
...
*Capstone-bound, Spring 2027*

---

> *"The best AI tutor is one that adapts to the student, not the other way around."*